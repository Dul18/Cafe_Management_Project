...
router.post('/add',auth.authenticateToken,checkRole.checkRole,(res,req)=>{
    ...
    connection.query(query,[product.name,product.categoryId,product.description,product.price],(err,results)=>{
        if(!err){
            return res.statusCode(200).json({message:"Product added successfully"});
        }else{
...
router.patch('/uppdate',auth.authenticateToken,checkRole.checkRole,(req,res,next)=>{
    let product = req.body;
...
router.patch('/updateStatus',auth.authenticateToken,checkRole.checkRole,(req,res,next)=>{
    ...
    connection.query(query,[user.id,user.status],(err,results)=>{
        if(!err){
            if(results.affectedRows == 0){
                return res.status(404).json({message:"Product id does not found"});
            }
